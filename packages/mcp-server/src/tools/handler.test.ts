...
vi.mock('@seco/core', () => ({
  startIntakeSession: vi.fn().mockResolvedValue({
    id: 'sess-1',
    messages: [{ role: 'assistant', content: 'Tell me about an experience.' }],
    status: 'active',
    transcript: '',
    experience_id: null,
...
describe('handleTool', () => {
  it('start_intake_session returns session_id', async () => {
    ...
    const data = JSON.parse(result.content[0].text) as { session_id: string };
